...
class Program
{
    static void Main()
    {
        ...
        RunBurstLengthAnalysis(rs);
        RunCapabilityTest(rs);

        Console.WriteLine("\nSvi testovi završeni.");
        Console.ReadKey();
    }

    static void RunBasicCorrectionTest(ReedSolomon rs, byte[] data)
    {
    ...
    static void RunBurstLengthAnalysis(ReedSolomon rs)
    {
    ...
    static void RunCapabilityTest(ReedSolomon rs)
    {
    ...
    static void TestRS(ReedSolomon rs, byte[] data, int[] idx, int[] val)
    {
        byte[] encoded = rs.Encode(data);
        byte[] corrupted = (byte[])encoded.Clone();
